...
function isBoxFree(board: number[][], r0: number, c0: number, num: number): boolean {
    for (let i = 0; i < 3; i++) for (let j = 0; j < 3; j++) if (board[r0 + i][c0 + j] === num) return false;
    return true;
}

function generatePuzzle(difficulty: 'easy' | 'medium' | 'hard'): { initial: number[][]; solution: number[][] } {
    ...
    let removed: number;
    switch (difficulty) {
        case 'easy':   removed = 28; break;
        case 'medium': removed = 40; break;
...
const SudokuGameV3: React.FC = () => {
    ...
    const conflicts = useCallback((b: number[][], r: number, c: number): boolean => {
        ...
        for (let i = 0; i < 3; i++) for (let j = 0; j < 3; j++) if ((r0 + i !== r || c0 + j !== c) && b[r0 + i][c0 + j] === n) return true;
        return false;
    }, []);
    ...
    useEffect(() => { startNewGame(difficulty); }, [difficulty, startNewGame]);
    ...
    const placeNumber = useCallback((num: number) => {
        ...
        // Compute new pencil marks synchronously (avoid stale closure)
        ...
        if (pencilMode) {
            ...
            if (conflicts(nb, row, col)) {
                ...
                if (num === solution[row][col]) {
                    const mp = pencil.map(r => r.map(c => [...c]));
                    mp[row][col] = [];
                    for (let i = 0; i < 9; i++) { mp[row][i] = mp[row][i].filter(x => x !== num); mp[i][col] = mp[i][col].filter(x => x !== num); }
                    const r0 = Math.floor(row / 3) * 3, c0 = Math.floor(col / 3) * 3;
                    for (let i = 0; i < 3; i++) for (let j = 0; j < 3; j++) mp[r0 + i][c0 + j] = mp[r0 + i][c0 + j].filter(x => x !== num);
                    newPencil = mp;

                    // Win check
                    let full = true;
                    for (let rr = 0; rr < 9 && full; rr++) for (let cc = 0; cc < 9 && full; cc++) if (nb[rr][cc] !== solution[rr][cc]) full = false;
    ...
    const highlighted = selected ? (() => {
        ...
        const v = board[r][c];
        if (v !== EMPTY) for (let rr = 0; rr < 9; rr++) for (let cc = 0; cc < 9; cc++) { if (board[rr][cc] === v) s.add(`${rr},${cc}`); }
        return s;
    })() : new Set<string>();
    ...
    return (
        <div className={styles.container}>
            {/* Title */}
            <h2 className={styles.title}>Sudoku 3</h2>
            ...
            <div className={styles.topBar}>
                ...
                <button className={styles.ctrlBtn} onClick={() => startNewGame(difficulty)}>
                    New Game
                </button>
            </div>
