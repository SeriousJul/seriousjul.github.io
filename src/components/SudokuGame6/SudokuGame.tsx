...
function generateSudoku(diff: 'easy' | 'medium' | 'hard') {
    // Build a full valid board using backtracking from empty state
    ...
    const removed = diff === 'easy' ? 28 : diff === 'medium' ? 42 : 50;
    let init = b.map(r => [...r]);
    let cnt = 0;
    while (cnt < removed) {
    ...
    // Verify no conflicts in the initial puzzle (should be guaranteed, but safety check)
    for (let r = 0; r < 9 && !conflictsExist(init); r++)
        for (let c = 0; c < 9; c++) {
            if (init[r][c] !== EMPTY) continue;
            for (const n of [1,2,3,4,5,6,7,8,9])
                if (!isValid(init, r, c, n)) { /* conflict in solution space — OK */ }
        }

    return { initial: init, solution: sol };
}

function fillDiagonal(b: number[][]) {
    // Fill the 3 diagonal boxes with random valid numbers
    for (let i = 0; i < 9; i += 3)
        fillBoxSafe(b, i, i);
}

function fillBoxSafe(b: number[][], r0: number, c0: number) {
    const ns = shuffle([1,2,3,4,5,6,7,8,9]);
    let idx = 0;
    for (let i = 0; i < 3 && idx < 9; i++)
        for (let j = 0; j < 3 && idx < 9; j++)
            b[r0 + i][c0 + j] = ns[idx++];
}
...
function conflictsExist(board: number[][]): boolean {
    for (let r = 0; r < 9; r++)
        for (let c = 0; c < 9; c++)
            if (board[r][c] !== EMPTY)
                for (const n of [1,2,3,4,5,6,7,8,9])
                    if (n !== board[r][c] && isValid(board, r, c, n)) return true;
    return false;
}
...
function getNotesForCell(marks: number[][][], r: number, c: number): number[] {
    const { initial, solution } = _getSolutionRef();
    if (initial[r][c] !== EMPTY) return [];
    // Return candidates that are NOT already in the row/col/box
    const candidates = [1, 2, 3, 4, 5, 6, 7, 8, 9].filter(n => {
        if (!isValid(initial.map(row => [...row]), r, c, n)) return false;
        // Also check what's already in the user-filled cells (not initial)
        for (let x = 0; x < 9; x++) if (x !== c && boardState[r][x] === n && initial[r][x] === EMPTY) return false;
        for (let y = 0; y < 9; y++) if (y !== r && boardState[y][c] === n && initial[y][c] === EMPTY) return false;
        const sR2 = Math.floor(r / 3) * 3, sC2 = Math.floor(c / 3) * 3;
        for (let i = 0; i < 3; i++)
            for (let j = 0; j < 3; j++)
                if ((sR2 + i !== r || sC2 + j !== c) && boardState[sR2 + i][sC2 + j] === n && initial[sR2 + i][sC2 + j] === EMPTY) return false;
        return true;
    });
    return candidates;
}
...
let _getSolutionRef: () => { initial: number[][]; solution: number[][] };
...
const SudokuGame: React.FC = () => {
    ...
    const _ref = { initial: [] as number[][], solution: [] as number[][] };
    ...
    _getSolutionRef = () => ({ initial: _ref.initial, solution: _ref.solution });

    // Keep boardState in sync for candidate calculation
    useEffect(() => { if (board.length > 0) Object.assign(boardState, board.map(r => [...r])); }, [board]);
    ...
    const handleInput = useCallback((num: number) => {
        ...
        if (pencilMode && num >= 1 && num <= 9) {
            ...
            // Place digit — check conflicts first
            setBoard(prev => {
